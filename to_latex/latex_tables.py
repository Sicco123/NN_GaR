...
def make_latex_table( steps_ahead, quantile_levels, test_stats, experiment_names, M, date, sample_sizes):
    with open(f'simulation_results/{date}_{M}_stats.pkl', "rb") as f:
        results = pickle.load(f)

    K, T, L, S, N = results.shape
    ...
    for k in range(K):
        for t in range(T):
            for n in range(N):
                sub_result = results[k,t,:,:,n]
                df_res = pd.DataFrame(sub_result, columns= cols, index=rows)
                s = df_res.style.format(precision = 3)
                latex_tab = s.to_latex(hrules = True, position_float = "centering",  caption = f'{experiment_names[k]} - {test_stats[n]} (T = {sample_sizes[t]}, M = 10000)',
                                            label=f'tab: {experiment_names[k]} {test_stats[n][:3]}', position = 'h!')

                if k == 0 and t ==0 and n == 0:
                    write_append = 'wt'
                    ...
                    with open(f'simulation_results/{date}_{M}_latex.tex', f'{write_append}') as file:
                        ...
                        file.write('\n')
                elif k == K-1 and t==T-1 and n == N-1:
                    write_append = 'at'
                    with open(f'simulation_results/{date}_{M}_latex.tex', f'{write_append}') as file:
                        file.write(latex_tab)
                        file.write('\n')
