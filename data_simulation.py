...
for train_sample_size in train_sample_sizes:
    ...
    Path(f"{dir_to_store_simulation}/{train_sample_size}").mkdir(parents=True, exist_ok=True)
    ...
    for b in range(number_of_samples):
        ...
        realisations_predictors = np.loadtxt(f'{dir_to_store_simulation}/{train_sample_size}/{b}_{day}.csv',delimiter = ",")
        predictors = realisations_predictors[:,1:].T
        ...
        # simulated output
        sim_realisations = linear_model(beta, predictors, errors[:,b])
        ...
        for h in range((np.max(horizon_list)+1)):
            q_error_contribution +=  np.dot(np.expand_dims(normal_quantiles,axis=1) ,np.expand_dims((phi * np.power(beta, h-1)),axis=0))  #  len(quantile_levels) x len(beta) -> 7 x 4
            mean = beta @ (phi**h * predictors)
            ...
            Path(f"{dir_to_store_simulation}/{train_sample_size}_f_quantiles_{h}").mkdir(parents=True, exist_ok=True)

            np.savetxt(f'{dir_to_store_simulation}/{train_sample_size}_f_quantiles_{h}/{b}_{day}.csv', q_h.T,
                       delimiter=",")

        store data
        np.savetxt(f'{dir_to_store_simulation}/{train_sample_size}/{b}_{day}.csv',realisations_predictors,delimiter = ",")
